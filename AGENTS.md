...
config/crew-dispatch.json  optional crewmate dispatch profiles; LOCAL, gitignored; firstmate-maintained but human-editable natural-language rules that choose a per-task harness/model/effort/live-search profile (section 4). Inherited by secondmate homes
...
An optional `webSearch: true` profile field is a task-scoped Codex capability selection; pass it as `--web-search on`, and leave live search off for profiles that do not explicitly request current sources.
