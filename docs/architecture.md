...
Live web search is fail-closed rather than portable: only the verified Codex adapter accepts `--web-search on`, emits Codex's native `--search`, and records the enabled axis for relaunch.
