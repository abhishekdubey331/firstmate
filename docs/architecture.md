...
The heartbeat fleet-scan surfaces both an unsurfaced captain-relevant status the per-wake path missed and a live-but-idle or dead lane that has no captain-relevant status at all, so a worker that finished cleanly or died without writing is detected as silence rather than staying invisible.
That silence backstop skips a lane with a legitimate explanation already on record - a declared `paused:` external wait, a verified captain-held transfer, an open `needs-decision`/`blocked` decision, or a captain-relevant last status line the other path already owns - and skips secondmate lanes, whose routed status is the supervision signal rather than their pane.
Heartbeat backoff is capped at `FM_HEARTBEAT_MAX` only for an empty fleet; whenever any task has a recorded endpoint the cap is the much tighter `FM_HEARTBEAT_BUSY_MAX`, so a fleet with work in flight can never drift into a multi-hour blind spot.
