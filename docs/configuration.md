...
The shell scripts do not match those rules; firstmate chooses the best matching rule with judgment, resolves its profile object or array under the operating contract in `AGENTS.md` section 4 and `quota-array-dispatch`, and passes only concrete `--harness`, `--model`, `--effort`, and `--web-search` flags to `fm-spawn.sh`.
  ...
  "rules": [
    {
      ...
      "use": [
        { "harness": "<adapter>", "model": "<optional model>", "effort": "<low|medium|high|xhigh|max, optional>", "webSearch": "<true|false, optional>" }
      ],
      "why": "<optional rationale that helps firstmate choose>"
    }
  ],
  "default": [
    { "harness": "<adapter>", "model": "<optional model>", "effort": "<optional effort>", "webSearch": "<optional boolean>" }
  ]
}
```
...
Profile `model`, `effort`, and `webSearch` fields and rule `why` are optional.
An omitted model or effort means the selected harness uses its own default for that axis.
`webSearch: true` is accepted only with the Codex harness and makes `fm-spawn` emit Codex's verified `--search` flag for that task.
An omitted or false `webSearch` keeps live search off.
...
Malformed JSON, an empty or malformed rule/default array, an unverified harness, an effort value unsupported by that harness, or an invalid live-search selection is reported as `CREW_DISPATCH: invalid config/crew-dispatch.json - ...`; missing `jq` is reported through the normal `MISSING: jq` install-consent flow.
