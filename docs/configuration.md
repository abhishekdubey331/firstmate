...
FM_HEARTBEAT_MAX=7200   # heartbeat backoff cap for an EMPTY fleet only (no task with a recorded endpoint)
FM_HEARTBEAT_BUSY_MAX=  # heartbeat backoff cap whenever any task has a recorded endpoint; defaults to FM_HEARTBEAT itself, i.e. no backoff at all
