...
| `relaunch` | Replace the running agent with a new one in the same endpoint and worktree, on the exact recorded adapter or an explicitly chosen harness, model, effort, and live-search posture. | The new agent is alive on the recorded endpoint, and the durable record names the harness that is actually running. |
...
1. **Resolve the profile.**
   An explicit `--harness`, `--model`, `--effort`, or `--web-search` wins.
   ...
   A harness change resets model, effort, and live search unless they are named too, because capabilities chosen for one adapter do not transfer to another.
2. **Safe checkpoint.**
